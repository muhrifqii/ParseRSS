...
import com.github.muhrifqii.parserss.RSSImage
import com.github.muhrifqii.parserss.RSSImageObject
import com.github.muhrifqii.parserss.RSSItem
import com.github.muhrifqii.parserss.RSSItemObject
...
import com.github.muhrifqii.parserss.exception.ParseRSSException
...
sealed class ParsingMode(val nameToken: String) : ParsingModeOperation {
    ...
    class Read : ParsingMode("") {
         ...
         */
        operator fun <T : RSSObject> set(clazz: KClass<T>, valueSetter: (T?) -> Unit) {
            ...
            try {
                ...
                throw ParseRSSException(
                    "Error on casting ${mode.nameToken} element to ${clazz.simpleName}",
                    err
                )
            }
        }
         ...
         */
        operator fun <T : RSSObject> ParsingMode.get(clazz: KClass<T>): T? {
            try {
                ...
                throw ParseRSSException(
                    "Error on casting rss object to ${clazz.simpleName}",
                    err
                )
            }
        }
 ...
 */
operator fun <T : RSSObject> ParsingMode.set(clazz: KClass<T>, valueSetter: (T?) -> Unit) {
    when (this) {
        is ParsingMode.Read -> {
 ...
 * Get the last rss mode's object
 */
operator fun <T : RSSObject> ParsingMode.get(clazz: KClass<T>): T? = when (this) {
    is ParsingMode.Read -> {
        this[clazz]
    }
...
// Utility functions for ParsingMode
internal fun <T> LinkedHashMap<String, T>.lastValue(): T? {
    return if (isEmpty()) null else values.last()
}
