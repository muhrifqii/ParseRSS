...
import com.github.muhrifqii.parserss.utils.toIntOrZero
...
class ParserExecutor<T>(
    ...
    private fun parseNSAtom(element: ParseRSSElement) {
        when (element.name) {
            ...
            ParseRSSKeyword.LINK -> mode[LinkEnabledObject::class.java] = {
                val rel =
                    parser.getAttributeValue(XmlPullParser.NO_NAMESPACE, ParseRSSKeyword.ATTR_REL)
                        ?: ""
                val href =
                    parser.getAttributeValue(XmlPullParser.NO_NAMESPACE, ParseRSSKeyword.ATTR_HREF)
